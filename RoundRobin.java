...
//        float time = rows.get(0).getArrivalTime();
        float time = 0;
        float timeQuantum = this.getTimeQuantum();
        ...
        while (!rows.isEmpty())
        {
            ...
            if(row.getArrivalTime() > time){
                this.getTimeline().add(new Event("I", time, row.getArrivalTime() - time ));
                time = row.getArrivalTime();
            }
            float bt = (row.getBurstTime() < timeQuantum ? row.getBurstTime() : timeQuantum);
        ...
        for (Row row : this.getRows())
        {
            ...
            for (Event event : this.getTimeline())
            {
                if (event.getProcessName().equals(row.getProcessName()))
                {
                    ...
                    {
                        float w = event.getStartTime() - (float) map.get(event.getProcessName());
